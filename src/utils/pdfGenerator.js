...
export const generateReimbursementPDF = async (request) => {
  try {
    // Create a temporary container for the PDF content
    const tempContainer = document.createElement('div');
    tempContainer.style.position = 'absolute';
    ...
    tempContainer.innerHTML = `
      <div style="max-width: 800px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
        ...
        <div style="text-align: center; margin-bottom: 30px; border-bottom: 2px solid #333; padding-bottom: 20px;">
          <h1 style="color: #333; margin: 0; font-size: 28px;">IIT MANDI</h1>
          <h2 style="color: #666; margin: 10px 0; font-size: 20px;">REIMBURSEMENT REQUEST</h2>
          <p style="margin: 5px 0; color: #666;">Request ID: ${request._id || 'N/A'}</p>
          <p style="margin: 5px 0; color: #666;">Generated on: ${new Date().toLocaleDateString()}</p>
        </div>
        ...
        <div style="text-align: center; margin-bottom: 30px;">
          <span style="
            ...
            font-weight: bold;
            font-size: 14px;
            ${request.status === 'Approved - Finance' || request.status === 'Completed' ? 
              'background-color: #d1fae5; color: #065f46;' : 
              request.status === 'Rejected' ? 'background-color: #fee2e2; color: #991b1b;' : 
              'background-color: #fef3c7; color: #92400e;'}
          ">
        ...
        <div style="margin-bottom: 30px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <tr>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb; width: 30%;">Submitted By:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.studentId?.name || request.facultySubmitterId?.name || 'N/A'}</td>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb; width: 30%;">Role:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.studentId ? 'Student' : 'Faculty'}</td>
            </tr>
            <tr>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Email:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.studentId?.email || request.facultySubmitterId?.email || 'N/A'}</td>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Department:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.studentId?.department || request.facultySubmitterId?.department || 'N/A'}</td>
            </tr>
            <tr>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Category:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.category || 'N/A'}</td>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Amount:</td>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; font-size: 16px;">₹${request.amount ? parseFloat(request.amount).toFixed(2) : '0.00'}</td>
            </tr>
            <tr>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Date Submitted:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.createdAt ? new Date(request.createdAt).toLocaleDateString() : 'N/A'}</td>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Date Completed:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.updatedAt ? new Date(request.updatedAt).toLocaleDateString() : 'N/A'}</td>
            </tr>
          </table>
        </div>
        ...
        <div style="margin-bottom: 30px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb; width: 30%;">Description:</td>
              <td style="padding: 10px; border: 1px solid #ddd;">${request.description || 'N/A'}</td>
            </tr>
            ${request.items && request.items.length > 0 ? `
              <tr>
                <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold; background-color: #f9fafb;">Items:</td>
                <td style="padding: 10px; border: 1px solid #ddd;">
                  <ul style="margin: 0; padding-left: 20px;">
                    ${request.items.map(item => `<li>${item.description || 'N/A'} - ₹${item.amount ? parseFloat(item.amount).toFixed(2) : '0.00'}</li>`).join('')}
                  </ul>
                </td>
              </tr>
            ` : ''}
        ...
        <div style="margin-bottom: 30px;">
          ...
          ${request.facultyRemarks ? `
            <div style="margin-bottom: 15px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background-color: #f0fdf4;">
              <div style="margin-bottom: 10px;">
                <strong>Faculty Review:</strong>
                <span style="padding: 5px 12px; border-radius: 15px; font-weight: bold; font-size: 12px; background-color: #16a34a; color: white; margin-left: 10px;">
                  REVIEWED
                </span>
              </div>
              <p style="margin: 10px 0; padding: 10px; background-color: rgba(0,0,0,0.05); border-radius: 5px; font-size: 14px;"><strong>Remarks:</strong> ${request.facultyRemarks}</p>
            </div>
          ` : ''}
          ...
          ${request.auditRemarks ? `
            <div style="margin-bottom: 15px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background-color: ${
              request.status === 'Rejected' ? '#fef2f2' : '#f0fdf4'
            };">
              <div style="margin-bottom: 10px;">
                ...
                <span style="
                  ...
                  font-size: 12px; 
                  margin-left: 10px;
                  ${request.status === 'Rejected' ? 'background-color: #dc2626; color: white;' : 'background-color: #16a34a; color: white;'}
                ">
                  ${request.status === 'Rejected' ? 'REJECTED' : 'REVIEWED'}
                </span>
              </div>
              <p style="margin: 10px 0; padding: 10px; background-color: rgba(0,0,0,0.05); border-radius: 5px; font-size: 14px;"><strong>Remarks:</strong> ${request.auditRemarks}</p>
            </div>
          ` : ''}
            ...
            <div style="margin-bottom: 15px; padding: 15px; border: 1px solid #ddd; border-radius: 8px; background-color: ${
              ...
              <div style="margin-bottom: 10px;">
                ...
                <span style="
                  ...
                  ${request.status === 'Rejected' ? 'background-color: #dc2626; color: white;' : 'background-color: #16a34a; color: white;'}
                ">
                  ${request.status === 'Rejected' ? 'REJECTED' : request.status === 'Approved - Finance' || request.status === 'Completed' ? 'APPROVED' : 'REVIEWED'}
                </span>
              </div>
              <p style="margin: 10px 0; padding: 10px; background-color: rgba(0,0,0,0.05); border-radius: 5px; font-size: 14px;"><strong>Remarks:</strong> ${request.financeRemarks}</p>
            </div>
          ` : ''}
        </div>
    ...
    // Save the PDF
    const fileName = `ReimbursementRequest_${request._id}_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
