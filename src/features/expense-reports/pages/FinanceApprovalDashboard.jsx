  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      if (activeTab === 'pending') {
        ...
        // Filter to show only reports processed by this Finance user
        const myProcessedReports = data.filter(report => {
          const financeApproval = report.financeApproval;
          return financeApproval && 
                 financeApproval.approvedById === user._id &&
                 (report.status === 'Finance Approved' || 
                  (report.status === 'Rejected' && financeApproval.approved === false));
        });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">
          Review and process audited expense reports
        </p>
      </div>
          ...
          {filteredReports.map(report => (
            ...
            >
              ...
              <div className="text-xs text-gray-600 border-t pt-2 space-y-1">
                ...
                {report.auditApproval?.approved && (
                  <div>✓ Audit: {report.auditApproval.approvedBy}</div>
                )}
              </div>
            </div>
          ))}
