...
const SchoolChairOverview = () => {
  ...
  const fetchDashboardStats = async () => {
    setIsLoading(true);
    try {
      // Fetch pending reports (Faculty Approved -> waiting for School Chair)
      const { data: pendingData } = await API.get('/expense-reports?pending=true');
      
      // Fetch processed reports
      const { data: processedData } = await API.get('/expense-reports?processed=true');
      
      // Calculate statistics - only count reports this School Chair actually approved
      const approved = processedData.filter(r => 
        r.schoolChairApproval?.approved === true
      );
      
      const rejected = processedData.filter(r => 
        r.status === 'Rejected' && r.schoolChairApproval?.approved === false
      );
      
      // Get recent reports (last 5 from both pending and processed)
      const allReports = [...pendingData, ...processedData]
        .sort((a, b) => new Date(b.submissionDate || b.createdAt) - new Date(a.submissionDate || a.createdAt))
        .slice(0, 5);
      
      // Calculate total amount approved this month by this School Chair
      const currentMonth = new Date().getMonth();
      const currentYear = new Date().getFullYear();
      const approvedThisMonth = approved.filter(r => {
        if (!r.schoolChairApproval?.date) return false;
        const approvalDate = new Date(r.schoolChairApproval.date);
        return approvalDate.getMonth() === currentMonth && 
               approvalDate.getFullYear() === currentYear &&
               r.schoolChairApproval.approvedById === user._id;
      });
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div 
            ...
            onClick={() => navigate('/expense-reports/school-chair/approvals')}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => navigate('/expense-reports/school-chair/approvals?tab=processed')}
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="text-gray-700 font-medium">View Processed Reports</span>
                </div>
              </button>
            </div>
          </div>
                ...
                stats.recentReports.map((report) => (
                  ...
                  >
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(report.status)}`}>
                      {report.status === 'Faculty Approved' ? 'Pending' : report.status}
                    </span>
                  </div>
                ))
              )}
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg shadow-sm border border-blue-100 p-6">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{user?.department}</h3>
              <p className="text-sm text-gray-600">School/Department Chair</p>
            </div>
          </div>
        </div>
      </div>
...
export default SchoolChairOverview;
